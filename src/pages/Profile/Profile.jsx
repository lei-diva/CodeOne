...
class ProfilePage extends React.Component{
    ...
    componentWillReceiveProps(nextProps) {
      ...
      componentWillMount(){
        try {
          this.setState({userRef: this.props.location.state.userRef, currentUser: this.props.location.state.currentUser});
        } catch (error) {
    ...
    dbCall = () => {
        ...
        if (this.props.userRef && this.props.userRef != 'init'){
            this.props.userRef.collection("projects").get()
            .then(querySnapshot => {
    ...
    render(){
        ...
        if (this.state.currentUser && this.state.projects){
            ...
            project.name.toLowerCase().includes(this.state.searchField.toLowerCase()));

        }


        if (!this.state.projects || !this.state.userRef == 'init') {
            projectsload = <div className="loading-page"><PropagateLoader
            css={override}
            className="loader"
            size={20}
            color={"#38B7E0"}
          ...
          /></div>
            ...
            projectsload = <span className="no-project-text">You have no projects</span>
        }

        else {

            projectsload = <div>
            <ProjectList deleteProject={this.deleteProject} projects={filteredProjects}/>
            </div>
        }
        ...
        <div className="profilepage">
            ...
            <span className="update-text">Update your projects or create a new one</span>
            </div>
            </div>

            </div>
            <button className="new-project-button" onClick={()=> this.props.history.push('/playground')}>Create a New Project</button>
